...
class FileExplorer(tk.Tk):
    def __init__(self):
        ...
        self.file_list = ttk.Treeview(self.file_frame, columns=("Name", "Size"), show="headings", style="Custom.Treeview")
        ...
        button_properties = [
            ...
            ("Rename File(s)", self.rename_file_gui),
            ("Move File(s)", self.move_file_gui),
            ("Remove Folder", self.remove_folder_gui),
            ("Delete File(s)", self.delete_file_gui),
            ("Compress File(s)", self.compress_file_gui),
        ...
        # Search Entry
        self.search_entry = ttk.Entry(self.toolbar, font=("Helvetica", 12), width=30)
        self.search_entry.grid(row=0, column=2, padx=5, pady=5, columnspan=2, sticky="ew")
    ...
    def search_files(self, event=None):
        """Search files by name or extension in the current directory."""
        ...
        try:
            for file_name in os.listdir(self.current_directory):
                ...
                if (search_term in file_name.lower() or 
                    (search_term.startswith('.') and file_name.lower().endswith(search_term)) or 
